/**
 * VECTX V3 - Layer 3 Research Script
 * 
...
// PostgreSQL returns NUMERIC as strings - convert to numbers
...
async function insertCorrelations(
  ...
  for (const c of correlations) {
    // Find the target asset to get its ID
...
async function callGemini(prompt: string): Promise<{ text: string; tokens_input: number; tokens_output: number; tokens_thinking: number; estimated_cost_usd: number; duration_ms: number }> {
  ...
  return { text, tokens_input, tokens_output, tokens_thinking, estimated_cost_usd, duration_ms };
}
...
function buildResearchPrompt(
  ...
  // Filter assets by class for correlations
...
function parseGeminiResponse(response: string): {
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      // Validate and normalize correlations
      ...
      // Validate and normalize trading metrics
...
async function processAsset(
  ...
  let response: string;
  let tokenData: { tokens_input: number; tokens_output: number; tokens_thinking: number; estimated_cost_usd: number; duration_ms: number };
  try {
    ...
    await logL3ResearchRun(assetId, assetName, false, 0, undefined, msg);
    return { correlations: 0, success: false };
  }

  console.log(`  Gemini response length: ${response.length}`);
  console.log(`  Tokens: ${tokenData.tokens_input}in/${tokenData.tokens_output}out/${tokenData.tokens_thinking}think, cost: $${tokenData.estimated_cost_usd.toFixed(6)}`);
  ...
  await logL3ResearchRun(assetId, assetName, true, tokenData.duration_ms, tokenData);

  return { correlations: analysis.correlations.length, success: true };
}

async function logL3ResearchRun(
  assetId: string,
  assetName: string,
  success: boolean,
  latencyMs: number,
  ...
  try {
    await runSql(`
      INSERT INTO central.l3_research_runs 
      ...
      VALUES 
        ('${assetId}', '${assetName.replace(/'/g, "''")}', 'gemini-2.5-flash', ${latencyMs}, ${success},
         ...
         ${errorMessage ? `'${errorMessage.replace(/'/g, "''").slice(0, 500)}'` : 'NULL'})
    `);
  } catch (e) {
    console.error(`  Failed to log L3 research run: ${e}`);
  }
}
